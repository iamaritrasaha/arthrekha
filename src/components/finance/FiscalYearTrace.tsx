...
export default function FiscalYearTrace({ latestIndex = 2, compact = false }: FiscalYearTraceProps) {
  return (
    <div className={`${styles.trace} ${compact ? styles.compact : ''}`} role="img" aria-label="India financial year runs from April to March. Actual data is available through June 2026; later periods are not available.">
      ...
      <div className={styles.line} aria-hidden="true"><i style={{ width: `${(latestIndex / (MONTHS.length - 1)) * 100}%` }} /><b style={{ left: `${(latestIndex / (MONTHS.length - 1)) * 100}%` }} /></div>
