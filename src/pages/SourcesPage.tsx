...
export default function SourcesPage() {
  ...
  return <div className={styles.page}>
    ...
    <section className={styles.sourceGrid}>
      ...
      <article><span>{t('EXECUTION')}</span><h2>{t('Controller General of Accounts')}</h2><p>{t('Provisional cumulative Union Government actuals from the available fiscal-year reporting periods.')}</p><dl><dt>{t('Estimate state')}</dt><dd>{t('Provisional actual')}</dd><dt>{t('Latest period')}</dt><dd>{currentPeriodText('Through {period} {year}', metadata.latestPeriod, metadata.financialYear, t)}</dd><dt>{t('Audit status')}</dt><dd>{t('Unaudited / subject to CAG audit')}</dd><dt>{t('Compatible metrics')}</dt><dd>{t('10 execution series')}</dd></dl><a href="https://cga.nic.in/" target="_blank" rel="noreferrer">{t('Open CGA ↗')}</a></article>
    </section>
